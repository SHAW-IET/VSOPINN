...
# [配置] 统一文件保存路径
# =========================================================================
OUT_DIR = "./output/完善/重新计算记录边界2点+中心2点/5 优化测点+注意力"
...
def train(epoch):
    ...
    # 2) 离散索引（带 STE，可反传）
    idx_float = key_points_model(grid_size)  # [N,2] float
    ...
    for iteration, batch in enumerate(training_data_loader):
        ...
        voronoi_input = softVoronoi(
            ...
            key_positions,  # 连续坐标，梯度走 data-loss  取消     detach
            grid_size, 80)
        ...
        u_pred, v_pred, p_pred = pred_uvp  # 每个 (N,)
        ...
        for name, param in list(model.named_parameters()) + list(key_points_model.named_parameters()):  # 清零 nan
        ...
        torch.nn.utils.clip_grad_norm_(model.parameters(), CLIP_NORM)  # 钳制到 1 以内
        torch.nn.utils.clip_grad_norm_(key_points_model.parameters(), CLIP_NORM)

        for param in key_points_model.parameters():  # 再次清零 nan
...
if __name__ == '__main__':
    ...
    try:
        ...
        # 将 numpy 坐标转换回 raw 参数 (inverse sigmoid)
        with torch.no_grad():
            eps = 1e-6
        ...
        print(f"[Warn] 无法恢复最佳传感器位置: {e}。使用最终位置。")
