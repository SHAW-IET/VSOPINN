...
optimizer = optim.Adam([
    ...
    {'params': key_points_model.parameters(), 'lr': 5e-4}  # 关键点使用更高学习率
], lr=lr)
...
def train(epoch):
	...
	# # 使用直通估计器获取离散索引
	# key_indices_tensor = key_points_model(grid_size)
	#
	# # 转换为整数索引列表
	# key_indices = []
	# for pt in key_indices_tensor.detach().cpu().numpy():
	# 	y_idx = int(round(pt[0]))
	# 	x_idx = int(round(pt[1]))
	# 	key_indices.append((y_idx, x_idx))  # (行,列)索引



	# --- 关键点的离散索引（带直通估计器，仍可反向） ---
	indices_tensor = key_points_model(grid_size)  # [N,2], (x_idx, y_idx)
	key_indices_dyn = [(int(x), int(y))  # 转成 (iy, ix)
	...
	for iteration, batch in enumerate(training_data_loader):
		...
		# 将张量转换为 numpy数组用于插值
		coord_2d = coord_tensor[0]  # [2, ny, nx]
		sol_u_2d = sol_u[0, 0]
		sol_v_2d = sol_v[0, 0]
		sol_p_2d = sol_p[0, 0]
		...
		optimizer.zero_grad()
		output = model(coord_tensor, voronoi_input)           # (1,1,17,82)
		...
		for iy, ix in key_indices_dyn: 								# 关键点被硬编码   loss_data = 0
			# 预测值（注意边界填充）
	...
	if epoch % 100 == 0:
		...
		for i, pos in enumerate(key_points_model.positions.detach().cpu().numpy()):
			actual_pos = pos * [nx - 1, ny - 1]            # 乘 100回来  (y, x)的形式
		...
		# 使用整个网格而不是内部点切片
		X = coord_tensor[0, 0, :, :].cpu().numpy()  # 整个网格的X坐标
		Y = coord_tensor[0, 1, :, :].cpu().numpy()  # 整个网格的Y坐标
