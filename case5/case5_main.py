...
SAVE_DIR = BASE_DIR / 'output' / 'output_heat_baseline'
...
def train(epoch):
    ...
    for iteration, batch in enumerate(training_data_loader):
        ...
        sl = slice(1, -1)
        loss = criterion(residual[:, :, sl, sl], torch.zeros_like(residual[:, :, sl, sl]))
    ...
    return loss.item(), l2_error_t.item(), mse_t.item()
...
if __name__ == '__main__':
    ...
    model = USCNN(h, nx, ny, NvarInput, NvarOutput, 'ortho').to('cuda')
    ...
    for ep in range(1, nEpochs + 1):
        ...
        if ep % 100 == 0:
            np.savetxt(
                ...
                np.column_stack([losses, errors_t, mses_t]),
                delimiter=',',
                header='loss,err_t,mse_t',
                comments=''
            )
    ...
    output_csv_path = SAVE_DIR / 'best_temperature_field_reconstructed.csv'
    df_field = pd.DataFrame({
        'x': x_plot.flatten(),
        'y': y_plot.flatten(),
        'T_true': t_true.flatten(),
        'T_pred': t_pred.flatten(),
        'abs_error_T': t_err.flatten(),
    })
    df_field.to_csv(output_csv_path, index=False, float_format='%.18e')
    print(f"Success! temperature field CSV saved to: {output_csv_path}")
