...
SAVE_DIR = BASE_DIR / 'output' / 'output_heat_optVoronoi_CVT'
...
from voronoi_utils import select_key_indices, LearnableKeyPoints, compute_cvt_update
...
def train(epoch):
    ...
    for iteration, batch in enumerate(training_data_loader):
        ...
        if epoch > 0 and epoch % CVT_INTERVAL == 0 and iteration == len(training_data_loader) - 1:
            ...
            with torch.no_grad():
                ...
                curr_ref = T_ref.to(device)
                curr_pred = outputT.detach()
                density_map = torch.sqrt((curr_pred - curr_ref) ** 2).mean(dim=0).squeeze(0)
    ...
    if epoch % 100 == 0:
        ...
        curr_pos = key_points_model.get_normalized().detach().cpu().numpy()
        print("Current sensor positions (normalized y, x):")
        for i, pos in enumerate(curr_pos):
            print(f"  Point {i + 1}: ({pos[0]:.4f}, {pos[1]:.4f})")
        key_history.append(curr_pos)
