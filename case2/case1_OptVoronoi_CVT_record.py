...
from voronoi_utils import VaryGeoDataset_PairedSolutionOld, to4DTensor, LearnableKeyPoints, \
    VaryGeoDataset_PairedSolution, softVoronoi, compute_cvt_update
...
# [配置] 文件保存路径
# =============================================================================
save_dir = Path(r"./output/2026.1.28完善数据记录/5 优化测点+CVT")
...
def train(epoch):
    ...
    for iteration, batch in enumerate(training_data_loader):
        ...
        # =========================================================
        #  [NEW] CVT Sensor Optimization Module (Fixed Version)
        # =========================================================
        CVT_INTERVAL = 200  # 每 500 个 epoch 执行一次
        device = 'cuda'
...
if __name__ == '__main__':
    ...
    np.save(save_dir / f"keypoints_history.npy", key_hist_arr)

    np.savetxt(save_dir / f"time.txt", [dt])

    # =========================================================================
    # [修复核心] 训练结束，重新加载最佳模型，生成最终结果
    # =========================================================================
