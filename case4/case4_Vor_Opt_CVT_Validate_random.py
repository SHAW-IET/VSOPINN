...
INIT_SEED = 42
MANUAL_INIT_COORDS = [[0.2, 0.2], [0.8, 0.2], [0.5, 0.5], [0.5, 0.8]]
...
print(f"Detected {num_conditions} condition configurations。current mode: {RUN_MODE}")


if RUN_MODE == 'OPTIMIZE':
    print(f">>> Optimization mode: initializing sensor positions (strategy: {INIT_STRATEGY})...")
    ...
    if VALIDATE_USE_RANDOM:
        print(f">>> Validation mode: using randomly generated points as baseline (seed: {VALIDATE_SEED})...")
        g = torch.Generator()
        g.manual_seed(VALIDATE_SEED)

        initial_positions = torch.rand(4, 2, generator=g) * 0.9 + 0.05
    else:
        print(">>> Validation mode: using fixed optimal cluster centers...")
        initial_positions = torch.tensor(FIXED_SENSOR_CENTROIDS, dtype=torch.float32)

    print(f"Initial positions:\n{initial_positions}")

else:
    raise ValueError("RUN_MODE must be 'OPTIMIZE' or 'VALIDATE'")
...
def train(epoch):
    ...
    for case_idx in range(num_conditions):
        ...
        grid_pts = torch.stack([2 * idx_float[:, 1] - 1, 2 * idx_float[:, 0] - 1], dim=-1).view(1, -1, 1, 2)
        ...
        loss_data = (criterion(u_pred, u_ref) + criterion(v_pred, v_ref) + criterion(p_pred, p_ref))
        loss_case = loss_pde + loss_data


        if RUN_MODE == 'OPTIMIZE':
            loss_cvt = compute_cvt_update(
                coord_2d, u, v, p,
                Rc, Ru, Rv,
                key_positions,
                (ny, nx),
...
if __name__ == '__main__':
    ...
    out_dir = "./output/improved/10 with_data_points - optimized_data_points - multi_conditions - CVT - validation/random4"
    os.makedirs(out_dir, exist_ok=True)


    current_case_name = case_config[0]['name']

    if RUN_MODE == 'OPTIMIZE':
        save_prefix = "Opt"
    else:

        type_str = "Rand" if VALIDATE_USE_RANDOM else "Fixed"
        save_prefix = f"Val_{type_str}_{current_case_name}"
    ...
    for ep in range(1, nEpochs + 1):
        ...
        if err_mag < best_mag_error:
            ...
            vis_loader = loaders_dict[vis_idx]
            ...
            with torch.no_grad():
                ...
                coord_2d = coord_tensor[0]
                voronoi_input = softVoronoi(coord_2d, sol_u[0, 0], sol_v[0, 0], sol_p[0, 0], key_pos, (ny, nx), 80)
                ...
                U_pred = u_vis[0, 0, :, :].cpu().numpy()
                V_pred = v_vis[0, 0, :, :].cpu().numpy()
                P_pred = p_vis[0, 0, :, :].cpu().numpy()
                Mag_pred = np.sqrt(U_pred ** 2 + V_pred ** 2)

                U_true = vis_ref['U'][0, 0, :, :].cpu().numpy()
                V_true = vis_ref['V'][0, 0, :, :].cpu().numpy()
                P_true = vis_ref['P'][0, 0, :, :].cpu().numpy()
                Mag_true = vis_ref['Mag'][0, 0, :, :].cpu().numpy()
                ...
                csv_header = "x,y,u_true,v_true,p_true,u_pred,v_pred,p_pred,mag_true,mag_pred,abs_error_mag"
                csv_path = os.path.join(out_dir, f"{save_prefix}_Best_Field_Data.csv")

                try:
                    np.savetxt(csv_path, data_matrix, delimiter=',', header=csv_header, comments='')
                except PermissionError:
                    print("[Warn] CSV file locked, skipping save.")


                fig, axes = plt.subplots(1, 3, figsize=(18, 5))

                im0 = axes[0].contourf(X_np, Y_np, Mag_pred, levels=50, cmap='viridis')
                axes[0].set_title(f"Predicted Mag ({current_case_name})")
                plt.colorbar(im0, ax=axes[0])

                im1 = axes[1].contourf(X_np, Y_np, Mag_true, levels=50, cmap='viridis')
                axes[1].set_title(f"Reference Mag")
                plt.colorbar(im1, ax=axes[1])

                im2 = axes[2].contourf(X_np, Y_np, Err_mag, levels=50, cmap='hot')
                axes[2].set_title(f"Abs Error (Min: {Err_mag.min():.4f}, Max: {Err_mag.max():.4f})")
                plt.colorbar(im2, ax=axes[2])

                plt.suptitle(f"Best Result at Epoch {ep} (Mag Err={best_mag_error:.5f})")
                plt.savefig(os.path.join(out_dir, f"{save_prefix}_Best_Comparison.png"), dpi=100)
                plt.close(fig)

            if ep % 100 == 0:
                print(f"[*] New Best Found at Epoch {ep}: Mag Err = {best_mag_error:.5f}. Saved CSV & Image.")
        ...
        if ep % 100 == 0 or ep == nEpochs:
            try:
                ...
                print(f"\n[Warning] Failed to save log file (Epoch {ep})，the file may be in use。")
    ...
    if RUN_MODE == 'OPTIMIZE':
        ...
        np.savetxt(os.path.join(out_dir, "Opt_keypoints_history.csv"),
                   ...
                   comments='')
        print(f"Optimization trajectory saved。")
